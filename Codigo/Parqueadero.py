...
{current_time}""")
    ...
    def cambiarSeccion(self):
        if self.seccion_estado == True:
            ...
            for label in self.labelCarro:
                label.destroy()
            self.labelCarro.clear()
            
            for label in self.labelMoto:
                label.destroy()
            self.labelMoto.clear()
            
            for label in self.labelMR:
                label.destroy()
            self.labelMR.clear()
...
Ingreso: {nuevoVehiculo.horaEntrada}""")
                    temporal = """if movilidad_reducida == 1:
                        self.vehiculoSeleccionado = "Movilidad Reducida"
                    self.verSeccion()
                    self.piso_label.configure(text= f"Piso: {self.pisoSeleccionado}")
                    self.seccion_label.configure(text = f"Sección: {self.vehiculoSeleccionado}")"""
                    self.buscar_vehiculo2(placa)
                    self.agregar_carro_moto.set(self.opciones[0])
    ...
    def calcular_tarifa(self, vehiculo):
        tarifa_por_hora = 2000  # La tarifa por cada hora o fracción de hora
        ...
        # Convertir la diferencia en horas y redondear hacia arriba para cobrar cualquier fracción
        horas = diferencia.total_seconds() / (60 * 60)
        horas_cobradas = math.ceil(horas)
    
        # Calcular el costo
        costo = horas_cobradas * tarifa_por_hora
    ...
    def eliminarVehiculo(self):
        if self.resultadoBusqueda == None:
            ...
            if messagebox.askokcancel("Pagar", f"""El costo a pagar es de: {costo}
Tiempo: {diferencia_redondeada}"""):
                ...
                if vehiculo_eliminado:
                    ...
                    self.eliminar_carro.configure(state=ctk.DISABLED)
                    self.buscar_otrovehiculo()
                    if self.pisoSeleccionado != None or self.pisoSeleccionado != None:
                        self.estacionamiento()
...
Ingreso: {vehiculo.horaEntrada}""", font= ("Arial", 18))
    ...
    #Para despues de añadir un vehiculo y que no suelte el msg
    #Metodo del boton buscar vehiculo
    def buscar_vehiculo2(self, placa):
        vehiculo = self.buscar_vehiculoPaEliminar(placa)
        if vehiculo == None:
            messagebox.showerror("Vehículo no registrado", f"El vehículo con la placa {placa.upper()} no se encuentra registrado")
        else:
            self.informacion_vehiculo.configure(text=f"""Posición: {vehiculo.posicion}
Placa: {vehiculo.placa}
Tipo: {vehiculo.tipoVehiculo}
...
Ingreso: {vehiculo.horaEntrada}""", font= ("Arial", 18))
    ...
    def estacionamiento(self):
        ...
        if piso == 1 and vehiculo == "Carro":
            ...
            for i in range(80):  
                texto = str((contador % num_columnas) + 1)
            ...
            for i in range (10):
                letra = chr(65 + c)
                y = 0.185 + yc
                labelLetra = ctk.CTkLabel(master=self.panel1,
                                          text = letra,
                                          font = ("Arial", 20))
                labelLetra.place(relx = 0.06, rely = y, anchor = ctk.CENTER)
                self.labelCarro.append(labelLetra)
                c += 1
                yc += 0.087
        ...
        if piso == 1 and vehiculo == "Moto":
            ...
            for i in range (15):
                ...
                labelLetra = ctk.CTkLabel(master=self.panel1,
                                          text = letra,
                                          font = ("Arial", 20))
                labelLetra.place(relx = 0.07, rely = y, anchor = ctk.CENTER)
                self.labelMoto.append(labelLetra)
                c += 1
                yc += 0.055
        ...
        if piso == 1 and vehiculo == "Movilidad Reducida":
            ...
            for i in range (2):
                ...
                labelLetra = ctk.CTkLabel(master=self.panel1,
                                          text = letra,
                                          font = ("Arial", 20))
                labelLetra.place(relx = 0.28, rely = y, anchor = ctk.CENTER)
                self.labelMR.append(labelLetra)
                c += 1
                yc += 0.05
